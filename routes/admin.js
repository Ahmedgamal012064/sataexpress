...
router.get('/notifications', isLoggedIn,function(req, res, next) {
    ...
    Notification.find({type : { $exists: true, $ne: null }},(err , result)=>{
        ...
        res.render('notifications/index', { title: 'notifications',notifications : result,permission:permission, layout: 'layout/admin', success : success });
    }).populate('user');
});
       ...
       if(type == "renve"){
           ...
           Order.find({status : "finished", fatora : false,createdAt : {$gte: date_from, $lt: date_to}},(err , orders)=>{
                ...
                var stats2  =  count != 0 ? true  : false ;
                 Fatora.find({},(err , result)=>{
                        res.render('reports/index', { title: 'reports',success:success,stats2:stats2,fators:result,date_from:datefrom,date_to:dateto,count:count,price:price,permission:permission, layout: 'layout/admin' });
            ...
            Order.find({status : "finished", fatora : false,createdAt : {$gte: date_from, $lt: date_to}},(err , orders)=>{
                ...
                var stats  =  count != 0 ? true  : false ;
                 Fatora.find({},(err , result)=>{
                        res.render('reports/index', { title: 'reports',success:success,stats:stats,fators:result,date_from:datefrom,date_to:dateto,count:count,price:price,permission:permission, layout: 'layout/admin' });
                 }).populate('whocreate').populate('user');
            }).populate('delvery');
       }
});
...
router.get('/mswada/:date_from/:date_to', isLoggedIn,function(req, res, next) {
           ...
           Order.find({status : "finished",createdAt : {$gte: date_from, $lt: date_to}},(err , orders)=>{
                res.render('reports/details', { title: 'mswada',orders:orders,permission:permission, layout: 'layout/admin' });
           }).populate('delvery');
});
...
router.get('/create-mswada/:date_from/:date_to', isLoggedIn,function(req, res, next) {
           ...
           Order.updateMany({status : "finished", fatora : false,createdAt : {$gte: date_from, $lt: date_to}} , {$set : {fatora : true}},(err , orders)=>{});
...
router.get('/Profile', isLoggedIn,function(req, res, next) {
    ...
    var user = req.user;
        ...
        res.render('auth/profile', { title: 'Admin Profile',permission:permission,  layout: 'layout/admin' , user : user, banners : banners  , successbanner : successbanner });
    });
});
...
router.get('/banners', isLoggedIn,function(req, res, next){
    ...
    Banner.find({},(err , result)=>{
        ...
        res.render('admins/banners', { title: 'Banners',banners : result,permission:permission,  layout: 'layout/admin' , success : success });
    });
});
