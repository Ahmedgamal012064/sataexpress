...
router.get('/active/:id', isLoggedIn,function(req, res, next) {
   ...
   User.updateOne({_id:req.params.id}, {$set : {status : 1}},(error , result)=>{
        if(error){
            console.log(error );
            res.redirect('/');
            return ;
...
router.get('/ban/:id', isLoggedIn,function(req, res, next) {
   ...
   User.updateOne({_id:req.params.id}, {$set : {status : 2}},(error , result)=>{
        if(error){
            console.log(error );
            res.redirect('/');
            return ;
...
router.get('/create/:type', isLoggedIn,function(req, res, next) {
  ...
  res.render('users/create',{title : 'Create-User',error:errors,cats:catt,type:type,type2:type2,permission:permission, layout: 'layout/admin' });
  });
});
...
router.get('/edit/:type/:id', isLoggedIn,function(req, res, next) {
  ...
  User.findOne({_id:req.params.id},(err , result)=>{ // find({where(name : 'ahmed')},select('name email'),callback)
    ...
    if(req.params.type == 'user'){
      ...
      var cats;
      Countery.find({},(err , catt)=>{
    res.render('users/edit',{title : 'Edit-User',cats:catt,type:type,type2:type2,user : result,permission:permission, layout: 'layout/admin' });
      });
  });
});
...
router.get('/detail/:id', isLoggedIn,function(req, res, next) {
  var permission = req.user.permission;
  var type;
  var success = req.flash('success-detail');
  User.findOne({_id:req.params.id},(err , result)=>{
    if(err){
      console.log(err);
      res.redirect('/');
    ...
    if(result.type == "user"){
      ...
      Order.find({user:req.params.id},(err , orders)=>{
        Fatora.find({user:req.params.id},(err , fatores)=>{
            res.render('users/detail',{title : 'Details-User',success:success,fatores:fatores,orders : orders,type:type,user  : result,permission:permission, layout: 'layout/admin' });
        });
      });
      ...
      Order.find({delvery:req.params.id},(err , orders)=>{
        ...
        res.render('users/detail',{title : 'Details-User',success:success,orders : orders,type:type,user : result,permission:permission, layout: 'layout/admin' });
            //   });
      });
    }
  });
