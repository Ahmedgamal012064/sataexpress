    ...
    //get all users
     var permission = req.user.permission;
      ...
      User.find({type : type2},(err , result)=>{ // find({where(name : 'ahmed')},select('name email'),callback)
        ...
        res.render('users/index',{title:'All Users',type:type, delever:delever,users : result,permission:permission,layout: 'layout/admin' , success : success});
...
allpending =  function(req, res, next) {
      var permission = req.user.permission;
       User.find({status : 0},(err , result)=>{ // find({where(name : 'ahmed')},select('name email'),callback)
        ...
        console.log(result);
        var success = req.flash('success-active');
        res.render('users/wait',{title:'Pending',users : result,permission:permission,layout: 'layout/admin' , success : success});
    }).sort({createdAt:'asc'}).populate('countery');
